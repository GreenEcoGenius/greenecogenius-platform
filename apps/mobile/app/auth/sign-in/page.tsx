...
export default function SignInPage() {
  ...
  async function handleMagicLink() {
    if (!email) { toast.error(t('email')); return; }
    setLoading(true);
    ...
    <div className="flex min-h-screen flex-col bg-[#0A2F1F] px-6 py-12">
      <div className="mx-auto w-full max-w-sm flex-1 flex flex-col justify-center">
        <div className="mb-10 text-center">
          ...
          <p className="mt-2 text-sm text-[#F5F5F0]/60">{t('signIn')}</p>
        </div>
        ...
        <p className="mt-8 text-center text-sm text-[#F5F5F0]/60">
          {t('noAccount')}{' '}
          <Link href="/auth/sign-up" className="text-[#B8D4E3] underline">{t('signUp')}</Link>
        </p>
      </div>
    </div>
  );
