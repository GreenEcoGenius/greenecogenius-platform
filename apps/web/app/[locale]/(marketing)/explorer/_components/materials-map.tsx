...
import { regionToSlug } from './explorer-data';
...
function formatVol(t: number): string {
...
const REGION_PATHS: Record<string, string> = {
  ...
  Corse:
    'M 440 420 L 460 410 L 475 430 L 470 470 L 455 480 L 440 460 Z',
};
...
export function MaterialsMap({
  ...
  for (const row of regionStats) {
    ...
    volumeByRegion.set(row.region, current + Number(row.total_volume_tonnes));
  }
        ...
        {tooltip && (
          ...
          >
            <div className="rounded-lg bg-white px-3 py-2 text-center shadow-lg">
              ...
              <p className="text-primary text-xs font-bold">{formatVol(tooltip.volume)}</p>
            </div>
          </foreignObject>
        )}
      </svg>
