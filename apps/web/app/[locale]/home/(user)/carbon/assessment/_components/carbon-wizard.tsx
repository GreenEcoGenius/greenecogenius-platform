    ...
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#C2DED1] focus:ring-1 focus:ring-[#C2DED1]"
        />
        {unit && <span className="shrink-0 text-xs text-gray-400">{unit}</span>}
      </div>
      {help && <p className="text-[11px] text-gray-400">{help}</p>}
      {estimate && (
        <p className="text-[11px] font-medium text-[#2D8C6A]">{estimate}</p>
      )}
    </div>
  );
}
...
function ScopeTotal({ label, tonnes }: { label: string; tonnes: number }) {
  return (
    <div className="mt-4 rounded-lg bg-[#E6F2ED] p-3 text-center">
      <p className="text-xs font-medium text-[#224E3F]">{label}</p>
      <p className="text-2xl font-bold text-[#1A3D32]">{Math.round(tonnes * 10) / 10} tCO2e</p>
    </div>
  );
}
...
export function CarbonWizard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!isResults && (
        <div className="space-y-2">
          ...
          <div className="h-2 w-full overflow-hidden rounded-full bg-gray-200">
            <div className="h-full rounded-full bg-[#E6F2ED]0 transition-all" style={{ width: `${((step + 1) / STEPS.length) * 100}%` }} />
          </div>
        </div>
      )}
      ...
      {step === 0 && (
        <Card>
          <CardContent className="space-y-4 p-5">
            <div className="flex items-center gap-2">
              <Building2 className="h-5 w-5 text-[#2D8C6A]" />
            ...
            <div className="space-y-2 pt-2">
              ...
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={profile.has_fleet} onChange={(e) => setProfile((p) => ({ ...p, has_fleet: e.target.checked }))} className="accent-[#2D8C6A]" />{fr ? 'Vehicules de fonction / flotte' : 'Company vehicles / fleet'}</label>
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={profile.has_refrigeration} onChange={(e) => setProfile((p) => ({ ...p, has_refrigeration: e.target.checked }))} className="accent-[#2D8C6A]" />{fr ? 'Climatisation ou refrigeration' : 'Air conditioning or refrigeration'}</label>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {step === 2 && (
        <Card>
          <CardContent className="space-y-5 p-5">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                {(['grid', 'renewable', 'self'] as const).map((src) => (
                  <label key={src} className={`flex items-center gap-2 rounded-lg border p-2.5 text-sm ${scope2.electricity_source === src ? 'border-[#C2DED1] bg-[#E6F2ED]' : 'border-gray-100'}`}>
                    <input type="radio" name="elec_source" value={src} checked={scope2.electricity_source === src} onChange={() => setScope2((p) => ({ ...p, electricity_source: src }))} className="accent-[#2D8C6A]" />
            ...
            <div className="rounded-lg border border-[#C2DED1] bg-[#E6F2ED]/50 p-3 text-xs text-[#224E3F]">
              {fr
      ...
      {isResults && result && (
        <div className="space-y-6">
          <Card className="border-[#C2DED1] bg-gradient-to-br from-[#E6F2ED] to-white">
            <CardContent className="flex flex-col items-center py-8">
              <p className="text-sm font-medium text-[#224E3F]">TOTAL</p>
              <p className="text-5xl font-bold text-[#1A3D32]">{Math.round(result.total * 10) / 10}</p>
              <p className="text-sm text-[#2D8C6A]">tCO2e</p>
