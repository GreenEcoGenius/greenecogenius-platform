...
export interface CertificateData {
  ...
  /** Base64-encoded PNG background generated by Flux (optional). */
  fluxBackgroundBase64?: string;
}
...
export async function generateCertificatePDF(
  ...
  // Flux AI background image (if available)
  if (data.fluxBackgroundBase64) {
    try {
      doc.addImage(
        data.fluxBackgroundBase64,
        'PNG',
        0,
        0,
        pw,
      ...
      // Semi-transparent white overlay so text remains readable
      doc.setFillColor(255, 255, 255);
      doc.setGState(new (doc as any).GState({ opacity: 0.82 }));
      doc.rect(0, 0, pw, ph, 'F');
      doc.setGState(new (doc as any).GState({ opacity: 1 }));
    } catch {
      // Background rendering failed — continue without it
    }
  }
