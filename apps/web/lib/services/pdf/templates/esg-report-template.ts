...
export interface ESGReportData {
  ...
  /** Flux-generated section illustrations (base64 PNG). Enterprise only. */
  fluxImages?: {
    environment?: string;
    social?: string;
    governance?: string;
...
/**
 * Insert a Flux-generated illustration into the PDF at the current Y position.
 * Returns the new Y position after the image and attribution line.
 */
function addFluxIllustration(
  doc: ReturnType<typeof pdfService.createDocument>,
  imageBase64: string,
  y: number,
  pw: number,
): number {
  try {
    const imgWidth = pw - 30; // 15mm margins
    const imgHeight = imgWidth * (800 / 1200); // 16:9 aspect ratio
    doc.addImage(imageBase64, 'PNG', 15, y, imgWidth, imgHeight);
    y += imgHeight + 2;
    doc.setFontSize(6);
    doc.setTextColor(...COLORS.gray);
    doc.text('Image generated by AI — Flux by Black Forest Labs', 15, y);
    y += 6;
  } catch {
    // Image rendering failed — skip silently
  }
  return y;
}
...
export function generateESGReportPDF(
  ...
  if (data.fluxImages?.carbon) {
    y = addFluxIllustration(doc, data.fluxImages.carbon, y, pw);
  }
  ...
  if (data.fluxImages?.circular_economy) {
    y = addFluxIllustration(doc, data.fluxImages.circular_economy, y, pw);
  }
